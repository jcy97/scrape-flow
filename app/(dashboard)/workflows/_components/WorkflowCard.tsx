...
} from "@/app/workflow/runs/[workflowId]/_components/ExecutionsStatusIndicator";
...
function WorkflowCard({ workflow }: { workflow: Workflow }) {
  ...
  hover:shadow-md dark:shadow-primary/30 group/card"
    >
      ...
      <LastRunDetials workflow={workflow} />
    </Card>
  );
}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <SchedulerDialog
        workflowId={workflowId}
        cron={cron}
        //cron이나 workflowId가 변경되면 컴포넌트 초기화되도록 설정
        key={`${cron}-${workflowId}`}
      />
      <MoveRightIcon className="h-4 w-4 text-muted-foreground" />
      <TooltipWrapper content="Credit consuption for full run">
...
function LastRunDetials({ workflow }: { workflow: Workflow }) {
  const isDraft = workflow.status === WorkflowStatus.DRAFT;
  if (isDraft) {
    return null;
  }
